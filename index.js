...
function displayPlatforms(games) {
  ...
  $games.toArray().forEach(g => {
    // loop through the jquery object array of <li> elements
    ...
    if (game.platformIds.length > 0) {
      // if this game object has platform look them up
      ...
      fetchGameData(
        'platforms',
        `fields *; where id = (${platforms});`,
        response => {
          game.platforms = response; // take the response object and add it to the game in the games collection
          $game
            .find('.platforms')
            .text(game.platforms.map(p => p.name).join(', ')); // add a comma delimited list of platforms. You can also add a link to the platofrms website or in turn lookup the platform image url
        }
      );
    }
  });
...
function getGameHtml(game) {
 ...
 <h2>&nbsp;${game.title}</h2>`;
  ...
  if (game.coverImageId > 0) {
    html += `<div class="coverimg">${getImage(game)}</div>`;
  }
  ...
  if (game.platformIds > 0) {
    html +=
      '<div class="info"><strong>Platforms:</strong>&nbsp;<span class="platforms"></div></span>';
  }

  if (game.releaseDate != false) {
    html += `<div class="info"><strong>Release Date:</strong>&nbsp;<span class="releaseDate">${readableDate(
      game
    )}</span></div>`;
  }

  if (game.rating > 0) {
    html += `<div class="info"><strong>Average Rating:</strong>&nbsp;<span id="content">${roundRating(
      game
    )}/100</span></div>`;
  }

  if (game.summary.length > 0) {
    html += `<div class="info"><strong>Summary:</strong>&nbsp;<span class="content">${
      game.summary
    }</span></div>`;
  }

  if (game.story.length > 0) {
    html += `<div class="info"><strong>Storyline:</strong>&nbsp;<span class="content">${
      game.story
    }</span></div>`;
  } else if (game.summary.length == 0 && game.story.length == 0) {
    html +=
      '<div class="info"><strong class="noInfo">Sorry there is no info on this game. It is either unreleased or still in development.</strong></div>';
  }
...
function getImage(game) {
  ...
  return `<img src="${game.coverImg.src}"" width="300" alt="${
    game.title
  } cover art" style="vertical-align: top;">`;
}
...
function getVideoGames(query) {
  fetchGameData(
    `games`,
    `fields id, name, summary, storyline, cover, platforms, first_release_date, total_rating; where name~*"${query}"*;`,
    response => {
      ...
      fetchGameData(
        ...
        response => {
          ...
          games.forEach(
            g => (g.coverImg = images.find(i => i.gameId === g.id))
          );
  ...
  fetch(`${proxyUrl}${searchURL}/${url}`, {
    ...
    .catch(err => {
      ...
      $('#js-error-message').text(
        `There doesn't seem to be a game with that title, please try again. Error: ${
          err.message
        }`
      );
      $('#landing').empty(); //removes info text
